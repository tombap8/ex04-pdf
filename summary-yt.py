...
#  ((최종변경사항 정리))
#  에러 핸들링 변경: 기존에는 자막을 가져오지 못하면 
# st.error를 띄우고 None을 반환하여 요약 프로세스가 
# 중단되었습니다. 이를 수정하여 자막이 없는 경우 
# 에러를 띄우지 않고, 영상의 제목을 포함한 대체 텍스트를 
# 반환하도록 했습니다.
# 요약 프로세스 유지: 자막 추출에 실패하더라도 AI 모델에 
# "자막 추출이 불가능하므로 영상 제목을 바탕으로 주제를 
# 유추해달라"는 내용과 함께 제목이 전달됩니다. 따라서 
# 요약 프로세스는 중단되지 않고 정상적으로 실행됩니다.
# 라이브러리 버전 호환성 개선: YouTubeTranscriptApi의 
# 버전에 따라 반환되는 데이터 형식(객체 또는 딕셔너리)이 
# 달라 오류가 나던 부분도 안전하게 처리되도록 개선했습니다.
